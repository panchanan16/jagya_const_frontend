...
const usersSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(GET_REQUEST.pending, (state) => {
            ...
            fulfilledStateReducer(state, action, 'users', 'GET')
            ...
            fulfilledStateReducer(state, action, 'users', 'POST')
            ...
            fulfilledStateReducer(state, action, 'users', 'DELETE', 'pro_id')
        }).addCase(DELETE_REQUEST.rejected, (state, action) => {
            state.loading = false
            state.error = action.payload
        }).addCase(UPDATE_REQUEST.fulfilled, (state, action) => {
            fulfilledStateReducer(state, action, 'users', 'UPDATE', 'pro_id')
